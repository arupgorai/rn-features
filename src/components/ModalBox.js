...
const ModalBox = ({ filterModalSharedValue1, filterModalSharedValue2 }) => {
  const filterModalContainerAnimatedStyle = useAnimatedStyle(() => ({
    opacity: interpolate(filterModalSharedValue1.value, [SCREEN_HEIGHT, 0], [0, 1]),
    transform: [{ translateY: filterModalSharedValue1.value }],
  }));

  const filterModalBackgroundAnimatedStyle = useAnimatedStyle(() => ({
    opacity: interpolate(filterModalSharedValue2.value, [SCREEN_HEIGHT, 0], [0, 1]),
  }));
  ...
  const closeModalBox = () => {
    ...
    filterModalSharedValue1.value = withDelay(500, withTiming(SCREEN_HEIGHT, { duration: 100 }));
  };

  return (
    <Animated.View style={[styles.mainContainer, filterModalContainerAnimatedStyle]}>
      <TouchableWithoutFeedback onPress={closeModalBox}>
        <Animated.View style={[styles.backgroundContainer, filterModalBackgroundAnimatedStyle]} />
      </TouchableWithoutFeedback>
      <Animated.View style={[styles.contentContainer, filterModalContentAnimatedStyle]}>
